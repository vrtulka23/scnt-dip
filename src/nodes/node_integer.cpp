...
namespace dip {
  ...
  std::unique_ptr<BaseValue> IntegerNode::cast_scalar_value(const std::string value_input) {    
    if (dtype_prop[1]=="16") {
      if (dtype_prop[0]=="u")
	return std::make_unique<ScalarValue<unsigned short>>((unsigned short)std::stoi(value_input), BaseValue::DataType::UINT16);
      else
	return std::make_unique<ScalarValue<short>>((short)std::stoi(value_input), BaseValue::DataType::INT16);
    } else if (dtype_prop[1]=="32" or dtype_prop[1]=="") {
      if (dtype_prop[0]=="u")
	return std::make_unique<ScalarValue<unsigned int>>(std::stoi(value_input), BaseValue::DataType::UINT32);
      else
	return std::make_unique<ScalarValue<int>>(std::stoi(value_input), BaseValue::DataType::INT32);
    } else if (dtype_prop[1]=="64") {
      if (dtype_prop[0]=="u")
	return std::make_unique<ScalarValue<unsigned long long>>(std::stoull(value_input), BaseValue::DataType::UINT64);
      else
	return std::make_unique<ScalarValue<long long>>(std::stoll(value_input), BaseValue::DataType::INT64);
    } else {
      if (dtype_prop[0]=="u")
  ...
  std::unique_ptr<BaseValue> IntegerNode::cast_array_value(const std::vector<std::string>& value_inputs, const std::vector<int>& shape) {
	...
	return std::make_unique<ArrayValue<unsigned short>>(arr, shape, BaseValue::DataType::UINT16);
      } else {
	std::vector<short> arr;
	for (auto s: value_inputs) arr.push_back((short)std::stoi(s));
	return std::make_unique<ArrayValue<short>>(arr, shape, BaseValue::DataType::INT16);
      }
    } else if (dtype_prop[1]=="32" or dtype_prop[1]=="") {
      if (dtype_prop[0]=="u") {
	std::vector<unsigned int> arr;
	for (auto s: value_inputs) arr.push_back(std::stoul(s));
	return std::make_unique<ArrayValue<unsigned int>>(arr, shape, BaseValue::DataType::UINT32);
      } else {
	std::vector<int> arr;
	for (auto s: value_inputs) arr.push_back(std::stoi(s));
	return std::make_unique<ArrayValue<int>>(arr, shape, BaseValue::DataType::INT32);
      }
    } else if (dtype_prop[1]=="64") {
      if (dtype_prop[0]=="u") {
	std::vector<unsigned long long> arr;
	for (auto s: value_inputs) arr.push_back(std::stoull(s));
	return std::make_unique<ArrayValue<unsigned long long>>(arr, shape, BaseValue::DataType::UINT64);
      } else {
	std::vector<long long> arr;
	for (auto s: value_inputs) arr.push_back(std::stoll(s));
	return std::make_unique<ArrayValue<long long>>(arr, shape, BaseValue::DataType::INT64);
      }
    } else {
